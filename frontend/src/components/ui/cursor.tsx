...
const MouseTrackerContext = React.createContext<MouseTrackerContextType | undefined>(undefined)
...
function MouseTrackerProvider({ ref, children, ...rest }: MouseTrackerProviderProps) {
    ...
    <AnimatePresence>
      {active && (
        <motion.div
          ...
          className={cx(
            'pointer-events-none absolute z-[9999] -translate-x-1/2 -translate-y-1/2 transform',
            className,
          )}
    ...
    <AnimatePresence>
      {active && (
        <motion.div
          ...
          className={cx(
            'pointer-events-none absolute z-[9998] -translate-x-1/2 -translate-y-1/2 transform font-medium',
            className,
          )}
          style={{ top: y, left: x, ...style }}
          initial={{ scale: 0, opacity: 0 }}
